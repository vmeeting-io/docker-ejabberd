...
-include_lib("xmpp/include/muc_roominfo.hrl").
...
do_decode([#xdata_field{var = <<"muc#roominfo_meetingId">>,
			values = [Value]}
	   | Fs],
	  XMLNS, Required, Acc) ->
    try Value of
      Result ->
	  do_decode(Fs, XMLNS,
		    lists:delete(<<"muc#roominfo_meetingId">>, Required),
		    [{meetingId, Result} | Acc])
    catch
      _:_ ->
	  erlang:error({?MODULE,
			{bad_var_value, <<"muc#roominfo_meetingId">>, XMLNS}})
    end;
do_decode([#xdata_field{var = <<"muc#roominfo_meetingId">>,
			values = []} =
	       F
	   | Fs],
	  XMLNS, Required, Acc) ->
    do_decode([F#xdata_field{var = <<"muc#roominfo_meetingId">>,
			     values = [<<>>]}
	       | Fs],
	      XMLNS, Required, Acc);
do_decode([#xdata_field{var = <<"muc#roominfo_meetingId">>}
...
encode_meetingId(Value, Lang, IsRequired) ->
    ...
    Opts = [],
    #xdata_field{var = <<"muc#roominfo_meetingId">>,
		 values = Values, required = IsRequired,
		 type = 'text-single', options = Opts, desc = <<>>,
		 label =
		     xmpp_tr:tr(Lang,
