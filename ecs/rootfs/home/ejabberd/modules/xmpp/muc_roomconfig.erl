...
-include_lib("xmpp/include/muc_roomconfig.hrl").
...
-spec encode(form(), binary(),
	     [maxhistoryfetch | allowpm | allow_private_messages |
	      allow_private_messages_from_visitors |
	      allow_visitor_status | allow_visitor_nickchange |
	      allow_voice_requests | allow_subscription |
	      voice_request_min_interval | captcha_protected |
	      captcha_whitelist | allow_query_users | allowinvites |
	      changesubject | enablelogging | getmemberlist | lang |
	      pubsub | maxusers | membersonly | moderatedroom |
	      members_by_default | passwordprotectedroom |
...
encode(List, Lang, Required) ->
    Fs = [case Opt of
		...
		[encode_persistentroom(Val, Lang,
				       lists:member(persistentroom, Required))];
...
do_decode([#xdata_field{var = <<"muc#roomconfig_meetingId">>,
			values = [Value]}
	   | Fs],
	  XMLNS, Required, Acc) ->
    try Value of
      Result ->
	  do_decode(Fs, XMLNS,
		    lists:delete(<<"muc#roomconfig_meetingId">>, Required),
		    [{meetingId, Result} | Acc])
    catch
      _:_ ->
	  erlang:error({?MODULE,
			{bad_var_value, <<"muc#roomconfig_meetingId">>, XMLNS}})
    end;
do_decode([#xdata_field{var = <<"muc#roomconfig_meetingId">>,
			values = []} =
	       F
	   | Fs],
	  XMLNS, Required, Acc) ->
    do_decode([F#xdata_field{var = <<"muc#roomconfig_meetingId">>,
			     values = [<<>>]}
	       | Fs],
	      XMLNS, Required, Acc);
do_decode([#xdata_field{var = <<"muc#roomconfig_meetingId">>}
...
do_decode([#xdata_field{var = <<"mam">>,
    ...
    try dec_bool(Value) of
      Result ->
	  do_decode(Fs, XMLNS, lists:delete(<<"mam">>, Required),
		    [{mam, Result} | Acc])
    catch
      _:_ ->
	  erlang:error({?MODULE,
...
-spec encode_meetingId(binary() | undefined, binary(),
		  boolean()) -> xdata_field().

encode_meetingId(Value, Lang, IsRequired) ->
    Values = case Value of
	       undefined -> [];
	       Value -> [Value]
	     end,
    Opts = [],
    #xdata_field{var = <<"muc#roomconfig_meetingId">>,
		 values = Values, required = IsRequired,
		 type = 'text-single', options = Opts, desc = <<>>,
		 label =
		     xmpp_tr:tr(Lang,
